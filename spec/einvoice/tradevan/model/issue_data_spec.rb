...
RSpec.describe Einvoice::Tradevan::Model::IssueData, type: :model do
  subject { build(:tradevan_issue_data) }
  ...
  context "validations" do
    ...
    # it { is_expected.to validate_presence_of(:transactionSource) }
    # it { is_expected.to validate_presence_of(:transactionTarget) }
    ...
    it { is_expected.to validate_length_of(:companyUn).is_equal_to(2) }
    it { is_expected.to validate_length_of(:orgId).is_equal_to(5) }
    # it { is_expected.to validate_length_of(:orgUn).is_equal_to(1) }
    it { is_expected.to validate_length_of(:type).is_at_most(8) }
    ...
    it { is_expected.to validate_length_of(:InvoicePaperReturned).is_equal_to(1) }
